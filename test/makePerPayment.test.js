...
describe("Landing", function () {
    ...
    //##########################################################################################
      ...
      let Amountfee;
      const lendingAmount = ethers.utils.parseEther('4');
    ...
    //   const sendSomeERC20 = await u20.connect(lender).transferFrom(lender.address, borrower.address, ethers.utils.parseEther('10')  );
    //   await sendSomeERC20.wait();
...
// create signture ##########################################################################################
  ...
  await  swopXLanding.connect(owner).balanceOf(borrower.address).then(res=>{
    ...
    const borrowerAmountApprove = ethers.utils.parseEther('10');
    const borrowerfee = Amountfee
    console.log("________________________________________________________________");
...
// payFirstMonth is the leaf of the 2nd month
    ...
    const loanTimestampLoanPayment = ["1656094663", "1666666666670000000"]
    ...
    const perPayLoan = merkleTree.getHexProof(leaf[0]);
    makePerPaymentloanTimestampLoanPayment = ["1656094663", "6000000000000000000"];
   
    const makePerPayment = await swopXLanding.connect(borrower).makePerPayment(nftreceipt, 
        makePerPaymentloanTimestampLoanPayment,borrowerfee,perPayLoan);
