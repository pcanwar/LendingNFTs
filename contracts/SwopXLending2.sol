...
contract SwopXLendingAssets is EIP712 {
    ...
    */
    ...
    function _hashLending(uint256 nonce,
    address paymentContract, uint256 loanCost, address nftcontract,
    address nftOwner, uint256 nftTokenId, uint256 pirodOfTime, uint256 cost,uint256 offeredTime) 
    public view returns (bytes32)
    {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("Landing(uint256 nonce,address paymentContract,uint256 loanCost,address nftcontract,address nftOwner,uint256 nftTokenId,uint256 pirodOfTime,uint256 cost,uint256 offeredTime)"),
            nonce,
            paymentContract,
            loanCost,
            nftcontract,
...
contract SwopXLending is Ownable, ReentrancyGuard, IERC721Receiver, SwopXLendingAssets, Pausable {
    ...
    address _nftcontract,address nftowner, uint256 tokenID,uint256 loanTerm,uint256 cost,uint256 offeredTime, bytes calldata signature) private view {
        require(_verify(verifer, _hashLending(nonce,
            paymentContract,_loanAmount, _nftcontract,
            nftowner,tokenID,loanTerm,cost,offeredTime)
            ,signature),"signature"); 
    }
   ...
   function submit(uint256 nonce, address _paymentAddress, address _lender, 
                    address _nftcontract, uint256 _nftTokenId, uint256 _loanAmount, 
                    uint256 _loanTerm, uint256 _loanCost, uint256 _offeredTime, 
                    bytes32 _root, bytes calldata signature) 
        public whenNotPaused nonReentrant
        supportInterface(_paymentAddress) expiredTime(_loanTerm)  {
        ...
        loanCost: _loanCost,
        payAmountAfterLoan:_loanAmount + _loanCost,
        payBackAfterLoan:0,
        isPaid:false,
        lender:_lender,
        ...
        verifyIt(_lender, nonce, _paymentAddress,_loanAmount,
        _nftcontract, msg.sender,_nftTokenId,_loanTerm,_loanCost, _offeredTime, signature);
        uint256 _time = clockTimeStamp();
        uint256 _timeRequired = calculatedTimeExpired(_loanTerm) ; // can be 730 hours per 1 month
        uint256 _payAmountAfterLoan = _loanAmount + _loanCost;
        uint256 counterId = counter();
        _transferNft(_nftcontract, msg.sender , _nftTokenId);
    
        uint256 fees_ = calculatedFee(_loanAmount);
        address contractOwner = owner();
        _assets[counterId].payAmountAfterLoan = _payAmountAfterLoan;
        _assets[counterId].listingTime = _time;
        _assets[counterId].loanTerm = _timeRequired + _time; // _timeRequired

        // LendingAssets memory _m = _assets[counterId];
        IERC20(_m.paymentContract).safeTransferFrom(_m.lender, contractOwner, fees_);
    ...
    function makePayment(uint256 _counterId) public nonReentrant {
